...
//-------------------------------------------------------------------------------incrementerCompteur
unsigned incrementerCompteur(void) {
...
int main(void) {
 ...
 futurs[1] = std::async(std::launch::async, [&]() {
  for_parallele<2>(0,taille, [&](unsigned i) { b[i] = c[i]/d[i]; } );
 });

 for_parallele<2>(0,taille, [&](unsigned i) { c[i] = e[i]*f[i]; } );

 for (unsigned i = 0; i<2; ++i) futurs[i].wait();

 for_parallele<6>(0,taille, [&](unsigned i) { g[i] = a[i] + b[i] - c[i]; } );
